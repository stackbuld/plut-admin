...
import {
  ...
  Dash,
  TablePager,
} from "@/components/plut/catalog-shared";

const DEFAULT_PAGE_SIZE = 20;

/**
 * Shared rates table — the single surface for viewing and updating denomination rates.
 *
 * Used by:
 *  - Catalog → Rates tab (all filters)
 *  - Card (brand) detail page → pass `lockBrandId` to lock to one card and hide the
 *    brand & country filters; pass `enableDenomActions` to expose activate/deactivate.
 */
export function RatesTable({
  lockBrandId,
  enableDenomActions = false,
}: {
  lockBrandId?: string;
  ...
  const params = {
    ...
    ...(!locked && countryFilter !== "All" && { CountryId: countryFilter }),
  ...
  const { data: brandsData } = useQuery({ ...brandQueries.list(), enabled: !locked });
  const { data: countriesData } = useQuery({ ...countryQueries.list(), enabled: !locked });
  ...
  }, [data?.totalCount, page, pageSize, lockBrandId, brandFilter, countryFilter, activeOnly]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const viewSym =
    view === "USD"
      ? "$"
      : currencySymbol(view) || activePayout.find((p) => p.code === view)?.symbol || view;
  const usdHeaders = [
    "Card",
    "Country",
    "Denom",
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        {!locked && (
          <>
            <FilterSelect
              ...
              options={[
                { v: "All", l: "All cards" },
                ...(brandsData ?? []).map((b) => ({ v: b.id, l: b.name })),
              ]}
            />
            <FilterSelect
              value={countryFilter}
              ...
              options={[
                ...
                ...(countriesData ?? []).map((c) => ({ v: c.id, l: c.name })),
              ]}
            />
          </>
        )}
        ...
        <span className="ml-auto text-xs text-muted-foreground">
          {total.toLocaleString()} rate{total === 1 ? "" : "s"}
        </span>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border bg-card overflow-hidden">
                    ...
                    >
                      ...
                      {view === "USD" ? (
                        <>
                          <td className="px-6 py-3.5 text-right font-mono">
                            {r.hasRate ? `$${r.marketRateUsd.toFixed(3)}` : <Dash />}
                          </td>
                          <td className="px-6 py-3.5 text-right font-mono">
                            {r.hasRate ? `$${r.customerRateUsd.toFixed(3)}` : <Dash />}
                          </td>
                          <td className="px-6 py-3.5 text-right font-mono">
                            {r.hasRate ? `${margin.toFixed(1)}%` : <Dash />}
                          </td>
                          <td className="px-6 py-3.5 text-xs text-muted-foreground">
                            {r.hasRate ? (r.rateSource === "Admin" ? "M" : "A") : "—"}
                          </td>
                          <td className="px-6 py-3.5 text-xs text-muted-foreground">
                        ...
                        <>
                          <td className="px-6 py-3.5 text-right font-mono">
                            {r.hasRate ? (
                              `${viewSym}${cost.toLocaleString("en-NG", { maximumFractionDigits: 0 })}`
                            ) : (
                              <Dash />
                            )}
                          </td>
                          <td className="px-6 py-3.5 text-right font-mono font-semibold">
                            {r.hasRate ? (
                              `${viewSym}${payout.toLocaleString("en-NG", { maximumFractionDigits: 0 })}`
                            ) : (
                              <Dash />
                            )}
                          </td>
                          <td className="px-6 py-3.5 text-right font-mono text-emerald-600 dark:text-emerald-400">
                            {r.hasRate ? (
                              `+${viewSym}${marginCur.toLocaleString("en-NG", { maximumFractionDigits: 0 })}`
                            ) : (
                              <Dash />
                            )}
                          </td>
                      ...
                      <td className="px-6 py-3.5 text-right">
                        <div className="inline-flex items-center gap-1">
                          ...
                          <Button
                            size="sm"
                            variant={r.hasRate ? "outline" : "default"}
                            onClick={() => setRateFor(ctx)}
                          >
                            {r.hasRate ? "Update" : "Set Rate"}
                          </Button>
                          {(r.hasRate || enableDenomActions) && (
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end">
                                ...
                                {enableDenomActions && (
                                  <>
                                    {r.hasRate && <DropdownMenuSeparator />}
                                    <DropdownMenuItem
                                      onClick={() =>
                                        denomMutation.mutate({
                                          id: r.denominationId,
                                          active: r.denominationIsActive,
                                        })
                                      }
                                    >
                                      {r.denominationIsActive
          ...
          <p className="border-t border-border bg-secondary/30 px-6 py-2 text-[11px] text-muted-foreground">
            Legend: M = Manual, A = Auto · Use the History button to see past rates · Supplier Quote
            shows the input mode used.
          </p>
        </div>
      )}
